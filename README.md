...
If for some reason you wanted to re-compute the synthetic photometry (don’t think you’ll need to but just in case) you can run: python ysg_temp_estimation.py --stars 848 --cores 8 (or however many cores you want to use)

Downloading YSG lightcurves:
Not necessary for you to see the spectra, but if you want to do this in order to see the lightcurves of the stars, you can download the tar file that Ben Shappee sent us in an email on Aug 25 2025 and put the contents in a subdirectory off the main directory you should call /sub2025.
