...
def determine_use_flags_from_swift(obs_band_names, obs_mags, summary, computed_models, star_idx, coords, true_phot_band_counter, binary, common_bands):
    """
    Determine use_U, use_B, use_V flags based on SWIFT band residuals.
    
    Parameters:
        obs_band_names: Array of observed band names
        obs_mags: Array of observed magnitudes
        summary: Dictionary with median fit results for U_redder, B_redder, V_redder
        computed_models: DataFrame of synthetic photometry models
        star_idx: Star index
        coords, df_smc, df_lmc, df_smc_prefinal, df_lmc_prefinal: Star data
        binary: Boolean indicating if the star is a binary
        choose_surveys_options: List of chosen surveys
    Returns:
        Dictionary with 'use_U', 'use_B', 'use_V' boolean flags
    """
    
    # SWIFT band names
    swift_bands = ['uvw1mag_SWIFT', 'uvw2mag_SWIFT', 'uvm2mag_SWIFT']
    ...
    #     if ((smc_vis_binaries['RA'] == RA) & (smc_vis_binaries['DEC'] == dec)).any() or ((smc_opt_binaries['RA'] == RA) & (smc_opt_binaries['DEC'] == dec)).any():
    #         binary = True
    # if star_idx >= 377 and star_idx < 848:
    #     if ((lmc_vis_binaries['RA'] == RA) & (lmc_vis_binaries['DEC'] == dec)).any() or ((lmc_opt_binaries['RA'] == RA) & (lmc_opt_binaries['DEC'] == dec)).any():
    #         binary = True
    
    # Check which SWIFT bands have observations
    obs_band_names_list = list(obs_band_names)
    obs_swift_bands = [band for band in swift_bands if band in obs_band_names_list]
      
    # Check for insufficient photometry FIRST, before checking if fits succeeded
    if len(common_bands) <= 3:
        return {'use_U': np.nan, 'use_B': np.nan, 'use_V': np.nan,
                'final_teff_median': np.nan, 'final_logg_median': np.nan, 'final_av_median': np.nan,
                'final_chi2': np.nan, 'final_teff_mean': np.nan, 'final_teff_std': np.nan,
                'final_logT_mean': np.nan, 'final_logT_std': np.nan, 'final_logg_mean': np.nan,
                'final_av_mean': np.nan, 'final_logL_mean': np.nan, 'final_logL_std': np.nan
                }  # Not enough data to determine
    
    # Check if we have median parameters for each fitting type
    has_U_median = (('teff_median_U_redder' in summary) and 
                    ('logg_median_U_redder' in summary) and 
                    ('av_median_U_redder' in summary))
    has_B_median = (('teff_median_B_redder' in summary) and 
                    ('logg_median_B_redder' in summary) and 
                    ('av_median_B_redder' in summary))
    has_V_median = (('teff_median_V_redder' in summary) and 
                    ('logg_median_V_redder' in summary) and 
                    ('av_median_V_redder' in summary))
    ...
    median_U = {
        'teff': summary['teff_median_U_redder'],
        'logg': summary['logg_median_U_redder'],
        'av': summary['av_median_U_redder']
    }
    median_B = {
        'teff': summary['teff_median_B_redder'],
        'logg': summary['logg_median_B_redder'],
        'av': summary['av_median_B_redder']
    }
    median_V = {
        'teff': summary['teff_median_V_redder'],
        'logg': summary['logg_median_V_redder'],
        'av': summary['av_median_V_redder']
    }
    
    # Filter models by metallicity based on star index
    if star_idx < 377 or (star_idx >= 848 and star_idx < 1012):
        models_subset = computed_models[computed_models['metallicity'] == -0.75]
    else:
        models_subset = computed_models[computed_models['metallicity'] == -0.25]
    
    # Function to find closest model
    def find_closest_model(median_params, models_df):
        """Find model closest to median parameters"""
        # Compute distance to median in parameter space (normalized)
        teff_norm = (models_df['teff'].values - median_params['teff']) / 500.0  # normalize by ~500K
        logg_norm = (models_df['logg'].values - median_params['logg']) / 0.5  # normalize by ~0.5 dex
        av_norm = (models_df['av'].values - median_params['av']) / 0.1  # normalize by ~0.1
        
        distance = np.sqrt(teff_norm**2 + logg_norm**2 + av_norm**2)
        closest_idx = np.argmin(distance)
        return models_df.iloc[closest_idx]
    
    # Find closest models for each median
    model_U = find_closest_model(median_U, models_subset)
    model_B = find_closest_model(median_B, models_subset)
    model_V = find_closest_model(median_V, models_subset)
    
    # Default: if not enough SWIFT observations, use B by default
    if len(obs_swift_bands) < 2 and len(common_bands) > 3:
        ### add here maybe a constraint on including B or V redder based on the BEST chi2 for each fitting type
        if ('chi2_B_redder' in summary) and ('chi2_V_redder' in summary):
            if summary['chi2_B_redder'] > 100 * summary['chi2_V_redder']:
                return {'use_U': False, 'use_B': False, 'use_V': True,
                        'final_teff_median': summary['teff_median_V_redder'],
                        'final_logg_median': summary['logg_median_V_redder'],
                        'final_av_median': summary['av_median_V_redder'],
                        'final_chi2': summary['chi2_V_redder'], 
                        'final_teff_mean': summary['teff_mean_V_redder'],
                        'final_teff_std': summary['teff_std_V_redder'],
                        'final_logT_mean': np.log10(summary['teff_mean_V_redder']),
                        'final_logT_std': summary['teff_std_V_redder'] / (summary['teff_mean_V_redder'] * np.log(10)),  # propagate to log space
                        'final_logg_mean': summary['logg_mean_V_redder'],
                        'final_av_mean': summary['av_mean_V_redder'],
                        'final_logL_mean': summary['logL_mean_V_redder'],
                        'final_logL_std': summary['logL_std_V_redder']
                        }
            else:
                return {'use_U': False, 'use_B': True, 'use_V': False,
                        'final_teff_median': summary['teff_median_B_redder'],
                        'final_logg_median': summary['logg_median_B_redder'],
                        'final_av_median': summary['av_median_B_redder'],
                        'final_chi2': summary['chi2_B_redder'], 
                        'final_teff_mean': summary['teff_mean_B_redder'],
                        'final_teff_std': summary['teff_std_B_redder'],
                        'final_logT_mean': np.log10(summary['teff_mean_B_redder']),
                        'final_logT_std': summary['teff_std_B_redder'] / (summary['teff_mean_B_redder'] * np.log(10)),  # propagate to log space
                        'final_logg_mean': summary['logg_mean_B_redder'],
                        'final_av_mean': summary['av_mean_B_redder'],
                        'final_logL_mean': summary['logL_mean_B_redder'],
                        'final_logL_std': summary['logL_std_B_redder']
                        }
        else:
            # If chi2 stats don't exist, default to use_B
            return {'use_U': False, 'use_B': True, 'use_V': False,
                    'final_teff_median': summary['teff_median_B_redder'],
                    'final_logg_median': summary['logg_median_B_redder'],
                    'final_av_median': summary['av_median_B_redder'],
    ...
    elif len(obs_swift_bands) >= 2 and len(common_bands) > 3:
        # We have at least 2 SWIFT bands - compute residuals
        # Get observed SWIFT magnitudes
        obs_mags_dict = dict(zip(obs_band_names, obs_mags))
        obs_swift_mags = np.array([obs_mags_dict[band] for band in obs_swift_bands])
        
        # Get synthetic SWIFT magnitudes for each median model
        synth_U_swift = np.array([model_U[band] for band in obs_swift_bands])
        synth_B_swift = np.array([model_B[band] for band in obs_swift_bands])
        synth_V_swift = np.array([model_V[band] for band in obs_swift_bands])
        
        # Calculate residuals (average absolute difference)
        residuals_U = np.mean(np.abs(obs_swift_mags - synth_U_swift))
        residuals_B = np.mean(np.abs(obs_swift_mags - synth_B_swift))
        residuals_V = np.mean(np.abs(obs_swift_mags - synth_V_swift))
        
        if residuals_U < residuals_B and binary==False:
            # U is closer
            return {'use_U': True, 'use_B': False, 'use_V': False,
                    'final_teff_median': summary['teff_median_U_redder'],
                    'final_logg_median': summary['logg_median_U_redder'],
                    'final_av_median': summary['av_median_U_redder'],
                    'final_chi2': summary['chi2_U_redder'], 
                    'final_teff_mean': summary['teff_mean_U_redder'],
                    'final_teff_std': summary['teff_std_U_redder'],
                    'final_logT_mean': np.log10(summary['teff_mean_U_redder']),
                    'final_logT_std': summary['teff_std_U_redder'] / (summary['teff_mean_U_redder'] * np.log(10)),
                    'final_logg_mean': summary['logg_mean_U_redder'],
                    'final_av_mean': summary['av_mean_U_redder'],
                    'final_logL_mean': summary['logL_mean_U_redder'],
                    'final_logL_std': summary['logL_std_U_redder']
                    }
        else:
            # B is closer or equal, now check B vs V
            if residuals_B <= residuals_V or binary==True:
                return {'use_U': False, 'use_B': True, 'use_V': False,
                        'final_teff_median': summary['teff_median_B_redder'],
                        'final_logg_median': summary['logg_median_B_redder'],
                        'final_av_median': summary['av_median_B_redder'],
                        ...
                        'final_teff_std': summary['teff_std_B_redder'],
                        'final_logT_mean': np.log10(summary['teff_mean_B_redder']),
                        'final_logT_std': summary['teff_std_B_redder'] / (summary['teff_mean_B_redder'] * np.log(10)),
                        'final_logg_mean': summary['logg_mean_B_redder'],
                        'final_av_mean': summary['av_mean_B_redder'],
                        'final_logL_mean': summary['logL_mean_B_redder'],
                        'final_logL_std': summary['logL_std_B_redder']
                ...
                return {'use_U': False, 'use_B': False, 'use_V': True,
                        ...
                        'final_teff_std': summary['teff_std_V_redder'],
                        'final_logT_mean': np.log10(summary['teff_mean_V_redder']),
                        'final_logT_std': summary['teff_std_V_redder'] / (summary['teff_mean_V_redder'] * np.log(10)),
                        'final_logg_mean': summary['logg_mean_V_redder'],
                        'final_av_mean': summary['av_mean_V_redder'],
                        'final_logL_mean': summary['logL_mean_V_redder'],
                        'final_logL_std': summary['logL_std_V_redder']
...
def process_star_chunk_vectorized(star_indices_chunk, choose_surveys, computed_models, coords, df_smc, df_lmc, df_smc_prefinal, df_lmc_prefinal, smc_vis_binaries, smc_opt_binaries, lmc_vis_binaries, lmc_opt_binaries, gaia_bprp, iterations=1000):
    ...
    for i, star_idx in enumerate(star_indices_chunk):
        ...
        try:
            ...
            # this code works properly due to lines in determine_use_flags_from_swift but there may be stars that have an APASS or MCPS flag that do not actuallyt have that data
            choose_surveys_row = choose_surveys[(choose_surveys['RA'] == RA) & (choose_surveys['DEC'] == dec)].iloc[0]
            if choose_surveys_row['choose_MCPS'] == 1:
                choose_surveys_options.extend(['Umag_MCPS', 'Bmag_MCPS', 'Vmag_MCPS', 'Imag_MCPS'])
                if choose_surveys_row['choose_APASS'] == 1:
                    choose_surveys_options.extend(['Bmag_APASS', 'Vmag_APASS', 'gmag_APASS', 'rmag_APASS', 'imag_APASS'])
                if choose_surveys_row['choose_APASS'] == 0 and 'Bmag_MCPS' in obs_band_names:
                    if pd.isna(obs_mags[list(obs_band_names).index('Bmag_MCPS')]):
                        #could add a print statement here
                        choose_surveys_options.append('Bmag_APASS')
                if choose_surveys_row['choose_APASS'] == 0 and 'Vmag_MCPS' in obs_band_names: 
                    if pd.isna(obs_mags[list(obs_band_names).index('Vmag_MCPS')]):
                        #could add a print statement here
                        choose_surveys_options.append('Vmag_APASS')
            elif choose_surveys_row['choose_APASS'] == 1:
                choose_surveys_options.extend(['Bmag_APASS', 'Vmag_APASS', 'gmag_APASS', 'rmag_APASS', 'imag_APASS'])
                 # Add MCPS U, since APASS doesn't have U and we want to keep it if APASS is chosen
                choose_surveys_options.append('Umag_MCPS')
                if choose_surveys_row['choose_MCPS'] == 0 and 'Bmag_APASS' not in obs_band_names:
            ...
            common_bands = [band for band in choose_surveys_options if band in obs_band_names]
            ...
            if best_fits_U_redder:
                ...
                # Only compute statistics if we have valid values
                if len(teff_values_U_redder) > 0:
                    summary.update({
                    ...
                    summary.update({
                    'n_fits_U_redder': 0,
                    'teff_mean_U_redder': np.nan,
                    'teff_median_U_redder': np.nan,
                    'teff_std_U_redder': np.nan,
                    'teff_16perc_U_redder': np.nan,
                    'teff_50perc_U_redder': np.nan,
                    'teff_84perc_U_redder': np.nan,
                    'logT_mean_U_redder': np.nan,
                    'logT_median_U_redder': np.nan,
                    'logT_std_U_redder': np.nan,
                    'logL_mean_U_redder': np.nan,
                    'logL_median_U_redder': np.nan,
                    'logL_std_U_redder': np.nan,
                    'logL_16perc_U_redder': np.nan,
                    'logL_50perc_U_redder': np.nan,
                    'logL_84perc_U_redder': np.nan,
                    'logg_mean_U_redder': np.nan,
                    'logg_median_U_redder': np.nan,
                    'logg_std_U_redder': np.nan,
                    'logg_16perc_U_redder': np.nan,
                    'logg_50perc_U_redder': np.nan,
                    'logg_84perc_U_redder': np.nan,
                    'av_mean_U_redder': np.nan,
                    'av_median_U_redder': np.nan,
                    'av_std_U_redder': np.nan,
                    'av_16perc_U_redder': np.nan,
                    'av_50perc_U_redder': np.nan,
                    'av_84perc_U_redder': np.nan,
                    'chi2_U_redder_mean': np.nan,
                    'n_excluded_U_redder': 0,
                    'most_common_excluded_U_redder': np.nan
                })
            ...
            if best_fits_B_redder:
                ...
                if len(teff_values_B_redder) > 0:
                    ...
                    summary.update({
                    'n_fits_B_redder': 0,
                    'teff_mean_B_redder': np.nan,
                    'teff_median_B_redder': np.nan,
                    'teff_std_B_redder': np.nan,
                    'teff_16perc_B_redder': np.nan,
                    'teff_50perc_B_redder': np.nan,
                    'teff_84perc_B_redder': np.nan,
                    'logT_mean_B_redder': np.nan,
                    'logT_median_B_redder': np.nan,
                    'logT_std_B_redder': np.nan,
                    'logL_mean_B_redder': np.nan,
                    'logL_median_B_redder': np.nan,
                    'logL_std_B_redder': np.nan,
                    'logL_16perc_B_redder': np.nan,
                    'logL_50perc_B_redder': np.nan,
                    'logL_84perc_B_redder': np.nan,
                    'logg_mean_B_redder': np.nan,
                    'logg_median_B_redder': np.nan,
                    'logg_std_B_redder': np.nan,
                    'logg_16perc_B_redder': np.nan,
                    'logg_50perc_B_redder': np.nan,
                    'logg_84perc_B_redder': np.nan,
                    'av_mean_B_redder': np.nan,
                    'av_median_B_redder': np.nan,
                    'av_std_B_redder': np.nan,
                    'av_16perc_B_redder': np.nan,
                    'av_50perc_B_redder': np.nan,
                    'av_84perc_B_redder': np.nan,
                    'chi2_B_redder_mean': np.nan,
                    'n_excluded_B_redder': 0,
                    'most_common_excluded_B_redder': np.nan
                })
            ...
            if best_fits_V_redder:
                ...
                if len(teff_values_V_redder) > 0:
                    ...
                    summary.update({
                    'n_fits_V_redder': 0,
                    'teff_mean_V_redder': np.nan,
                    'teff_median_V_redder': np.nan,
                    'teff_std_V_redder': np.nan,
                    'teff_16perc_V_redder': np.nan,
                    'teff_50perc_V_redder': np.nan,
                    'teff_84perc_V_redder': np.nan,
                    'logT_mean_V_redder': np.nan,
                    'logT_median_V_redder': np.nan,
                    'logT_std_V_redder': np.nan,
                    'logL_mean_V_redder': np.nan,
                    'logL_median_V_redder': np.nan,
                    'logL_std_V_redder': np.nan,
                    'logL_16perc_V_redder': np.nan,
                    'logL_50perc_V_redder': np.nan,
                    'logL_84perc_V_redder': np.nan,
                    'logg_mean_V_redder': np.nan,
                    'logg_median_V_redder': np.nan,
                    'logg_std_V_redder': np.nan,
                    'logg_16perc_V_redder': np.nan,
                    'logg_50perc_V_redder': np.nan,
                    'logg_84perc_V_redder': np.nan,
                    'av_mean_V_redder': np.nan,
                    'av_median_V_redder': np.nan,
                    'av_std_V_redder': np.nan,
                    'av_16perc_V_redder': np.nan,
                    'av_50perc_V_redder': np.nan,
                    'av_84perc_V_redder': np.nan,
                    'chi2_V_redder_mean': np.nan,
                    'n_excluded_V_redder': 0,
                    'most_common_excluded_V_redder': np.nan
                })
            ...
            summary.update({
                ...
                'bands_used_for_fitting': ','.join(choose_surveys_options)  # Save which bands were used
            })
            ...
            # Determine use_U, use_B, use_V flags based on SWIFT data
            use_flags = determine_use_flags_from_swift(
                obs_band_names=obs_band_names,
                obs_mags=obs_mags,
                summary=summary,
                ...
                coords=coords,
                binary=binary,
                true_phot_band_counter=true_phot_band_counter,
                common_bands=common_bands
            )
            
            # Add use flags to summary
            summary.update(use_flags)
...
def compute_ysgs_parallel(total_star_indices, coords, df_smc, df_lmc, df_smc_prefinal, df_lmc_prefinal, smc_vis_binaries, smc_opt_binaries, lmc_vis_binaries, lmc_opt_binaries, gaia_bprp, choose_surveys, computed_models, n_cores=4, iterations=1000):
    """
...
def main():
    ...
    try:
        # Load data
        coords, df_smc, df_lmc, df_smc_prefinal, df_lmc_prefinal, smc_vis_binaries, smc_opt_binaries, lmc_vis_binaries, lmc_opt_binaries, gaia_bprp, choose_surveys, computed_models = load_data()
